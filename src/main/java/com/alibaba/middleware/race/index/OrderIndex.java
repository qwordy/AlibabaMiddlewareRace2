...
public class OrderIndex {
  ...
  private final int BUCKET_NUM = 1 << 14;
  ...
  // 14
  private final int SMALL_MASK = 0x3fff;
  ...
  public void add(byte[] id, int fileId, long fileOff) throws Exception {
    ...
    table[hash >> 14].add(id, hash & SMALL_MASK, fileId, fileOff);
  }

  public Tuple get(byte[] id) throws Exception {
    int hash = Util.getHashCode(id) & BIG_MASK;
    return table[hash >> 14].get(id, hash & SMALL_MASK);
  }
}
