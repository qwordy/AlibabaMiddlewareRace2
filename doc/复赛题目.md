...
每种类型的文件中有一些字段是一定存在的，且类型固定。查询操作主要通过以下字段进行：
...
1. queryOrder，依据orderid，查询指定订单号的交易信息，可以指定要查询的列
1. queryOrdersByBuyer，依据buyerid与createtime，查询买家某个时间范围内的交易信息
1. queryOrdersBySaler，依据salerid与goodid，查询卖家某个商品的交易信息，可以指定要查询的列
1. sumOrdersByGood，依据goodid，对商品的某个字段进行求和

查询的字段可能存在于三张表中，即，查询的数据是join后的数据
...
* Docker隔离出6核、10G内存的运行环境。
* 3块1.5T大小磁盘，数据均匀分布在3块磁盘上
* jvm 5G内存
